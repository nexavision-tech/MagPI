export const generatePythonScript = (nodes, connections, crs, processingScope) => {
    let script = `# Generated by MagPI Canvas (v0.1.45 Alpha)\n`;
    ...
    // --- MAGPI TOPOLOGICAL SORT (Kahn's Algorithm) ---
    // Reads the literal wires (edges) to figure out the exact execution order, 
    // giving you total freedom to place nodes anywhere on the screen!
    const adjList = {};
    const inDegree = {};
    nodes.forEach(n => {
        adjList[n.id] = [];
    ...
    const queue = [];
    // Start with nodes that have NO incoming wires (the source nodes)
    Object.keys(inDegree).forEach(id => {
        if (inDegree[id] === 0) queue.push(id);
    });

    // Fallback sort by X-axis just for visually organizing disjointed blocks
    ...
    sortedNodes.forEach((n) => {
        ...
        const inRasterVar = getInVar(['raster', 'sentinel2', 'elevation', 'nlcd', 'ndvi', 'pansharpen', 'project_raster', 'pyramids']) || 'None';
        const inLabelVar = getInVar(['nlcd', 'census', 'load_vector', 'pyramids']) || 'None';
        ...
        // 7. Data Management
            ...
            // FIX: Pass-through assignment so downstream nodes don't lose the variable reference!
