...
import { Code, X, Copy, Rocket, CheckCircle } from 'lucide-react';
...
export default function ScriptModal({ 
  ...
  const copyToClipboard = () => {
    ...
    // Could add a small toast notification here later!
  };
      ...
      <div className="bg-slate-800 rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.5)] w-full max-w-4xl border border-slate-600 flex flex-col overflow-hidden transform transition-all">
        ...
        <div className="p-6 bg-[#0d1117] flex-1 relative group">
          ...
          <button 
            ...
            className="absolute top-4 right-4 bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-md text-xs font-bold border border-slate-600 shadow-md opacity-0 group-hover:opacity-100 transition-opacity flex items-center" 
          >
        ...
        <div className="p-4 bg-slate-900 border-t border-slate-700 flex justify-between items-center">
          ...
          <div className="flex space-x-3">
            <button 
              onClick={() => setShowScript(false)} 
              className="px-5 py-2.5 rounded-md text-sm font-bold text-slate-300 hover:bg-slate-700 hover:text-white border border-transparent hover:border-slate-600 transition-all"
            >
              Close
            </button>
            <button 
              onClick={onDeploy} 
              className="px-5 py-2.5 rounded-md text-sm bg-emerald-600 hover:bg-emerald-500 text-white font-bold shadow-lg flex items-center transition-all hover:scale-105 active:scale-95"
            >
