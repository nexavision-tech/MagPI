      ...
      return (
          <div key={out.id} className={`w-full relative flex items-center justify-end transition-all duration-300 ${isHidden ? 'hidden' : 'h-4'}`}>
              {toolId === 'core_date_variable' && (out.id === 'start' || out.id === 'end') ? (
                  ...
                  <span style={{ color: color }} className="text-[9px] font-mono font-bold tracking-widest pointer-events-none drop-shadow-sm mr-1 truncate max-w-[120px] text-left w-full" title={out.label || out.id.toUpperCase()}>{out.label || out.id.toUpperCase()}</span>
              )}
