...
const MagPINode = ({ data, id }) => {
  ...
  React.useEffect(() => {
    if (toolId === 'load_raster' && data.params?.file_path && !data.metadataFetched) {
        fetch(`http://${window.location.hostname}:8080/api/raster_metadata?file=${encodeURIComponent(data.params.file_path)}`)
    ...
    if (toolId === 'load_vector' && data.params?.file_path && !data.metadataFetched) {
        fetch(`http://${window.location.hostname}:8080/api/vector_metadata?file=${encodeURIComponent(data.params.file_path)}`)
        .then(res => res.ok ? res.json() : null)
        .then(meta => {
            if (meta && !meta.error) {
                let fileExt = '';
