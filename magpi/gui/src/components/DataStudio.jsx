...
export default function DataStudio() {
  ...
  const fetchDatabases = async () => {
    try {
      const res = await fetch(`http://${window.location.hostname}:8080/api/databases`);
  ...
  const executeQuery = async () => {
    ...
    try {
      const res = await fetch(`http://${window.location.hostname}:8080/api/query`, {
        method: 'POST',
  ...
  const handleAddConnection = async () => {
    ...
    try {
      const res = await fetch(`http://${window.location.hostname}:8080/api/db_connections`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
