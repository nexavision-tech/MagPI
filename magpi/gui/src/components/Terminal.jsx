...
export default function Terminal({ showTerminal, setShowTerminal, logs, isProcessing, selectedNode }) {
  ...
  useEffect(() => {
      if (activeTab === 'db_studio') {
          fetch(`http://${window.location.hostname}:8282/api/db_connections`)
              .then(r => r.json())
  ...
  useEffect(() => {
      if (selectedDb) {
          setIsLoadingDb(true);
          setDbError(null);
          fetch(`http://${window.location.hostname}:8282/api/db_tables?connection=${encodeURIComponent(selectedDb)}`)
              .then(r => r.json())
              .then(data => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        ...
        setDataError(null);
        try {
            const res = await fetch(`http://${window.location.hostname}:8282/api/vector_data?file=${encodeURIComponent(path)}&limit=${limit}&offset=${page * limit}`);
