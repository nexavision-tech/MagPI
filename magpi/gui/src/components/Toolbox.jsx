...
// --- MOCK TOOLBOX DATA WITH ENHANCED PARAMETERS ---
  ...
  },
  {
    ...
    tools: [
      { id: 'ai_detect', name: "Detect Objects", type: 'process', icon: <Crosshair size={14}/>, color: 'bg-purple-600', border: 'border-purple-500', 
        params: { 
            out_shp: "pools.shp", 
            model: { value: "facebook/detr-resnet-50", type: "select", options: ["facebook/detr-resnet-50", "facebook/mask2former-swin-large-cityscapes-semantic", "nvidia/segformer-b0-finetuned-ade-512-512"] } 
        } 
      },
    ]
  },
  {
    ...
    tools: [
      ...
      { id: 'mgt_buffer', name: "Buffer", type: 'process', icon: <CircleDashed size={14}/>, color: 'bg-slate-600', border: 'border-slate-500', 
        params: { 
            distance: 50, 
            unit: { value: "Meters", type: "select", options: ["Meters", "Kilometers", "Feet", "Miles"] } 
        } 
      },
    ]
  }
...
export default function Toolbox({ 
  ...
  const [expandedCategories, setExpandedCategories] = useState({ 
    "Data Ingestion": true, "Image Analyst (ia)": true, "GeoAI (geoai)": true, "Data Management": true 
  });
  ...
  return (
    <div className="w-[320px] bg-slate-800 flex flex-col shadow-[-10px_0_20px_rgba(0,0,0,0.5)] z-20">
      ...
      <div className="flex bg-slate-900 border-b border-slate-700">
        <button onClick={() => setActiveRightTab('toolbox')} className={`flex-1 py-4 text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center ${activeRightTab === 'toolbox' ? 'text-emerald-400 border-b-2 border-emerald-400 bg-slate-800' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/50'}`}>
          <Wrench size={14} className="mr-2" /> Tools
        </button>
        <button onClick={() => setActiveRightTab('inspector')} className={`flex-1 py-4 text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center ${activeRightTab === 'inspector' ? 'text-emerald-400 border-b-2 border-emerald-400 bg-slate-800' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/50'}`}>
              ...
              <div key={idx} className="bg-slate-900/80 rounded-lg border border-slate-700/80 overflow-hidden shadow-sm">
                ...
                {expandedCategories[cat.name] && (
                  <div className="p-2 space-y-1 bg-slate-900/50">
                    {cat.tools.map(tool => (
                      <div key={tool.id} onClick={() => addNode(tool)} className="flex items-center px-3 py-2.5 text-xs bg-slate-800 hover:bg-slate-700 rounded-md cursor-pointer border border-transparent hover:border-emerald-500/50 transition-all group shadow-sm">
              ...
              <div className="space-y-4 animate-fadeIn">
                ...
                <div className="bg-slate-900 p-4 rounded-lg border border-slate-700 shadow-inner">
                  ...
                  {Object.entries(selectedNode.params || {}).map(([key, val]) => {
                    // Extract value if it is a complex object (like our new select dropdowns)
                    ...
                    <div key={key} className="mb-4">
                      ...
                      {typeof displayVal === 'boolean' ? (
                        <div className="flex items-center bg-slate-800 px-3 py-2 rounded-md border border-slate-700 cursor-pointer" onClick={() => updateNodeParam(selectedNode.id, key, !displayVal)}>
                          <div className={`w-4 h-4 rounded-sm flex items-center justify-center mr-3 transition-colors ${displayVal ? 'bg-emerald-500' : 'bg-slate-700 border border-slate-600'}`}>
                            {displayVal && <Check size={10} className="text-white" />}
                          </div>
                        ...
                        <div className="relative">
                            ...
                            <select 
                                value={displayVal} 
                                onChange={(e) => updateNodeParam(selectedNode.id, key, { ...val, value: e.target.value })}
                                className="w-full bg-slate-800 border border-slate-600 rounded-md pl-9 pr-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all font-mono appearance-none cursor-pointer"
                            >
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button className="p-2 bg-slate-700 hover:bg-slate-600 rounded border border-slate-600 transition-colors text-slate-300" title="Browse Server (Coming Soon)"><FolderOpen size={16} /></button>
                        </div>
                      ) : (
